...
#define DownloadDirectory self.downloadedFilesDirectory ?: [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] \
                                                               stringByAppendingPathComponent:NSStringFromClass([self class])]

#define SRFileName(URL) [URL lastPathComponent]

#define SRFilePath(URL) [DownloadDirectory stringByAppendingPathComponent:SRFileName(URL)]

#define SRFilesTotalLengthPlistPath [DownloadDirectory stringByAppendingPathComponent:@"FilesTotalLength.plist"]

@interface DownloadManager() <NSURLSessionDelegate, NSURLSessionDataDelegate>
...
- (instancetype)init {
    ...
    if (self = [super init]) {
        NSString *downloadDirectory = DownloadDirectory;
        BOOL isDirectory = NO;
               ...
               completion:(void(^)(BOOL success, NSString *filePath, NSError *error))completion
{
    ...
    DownloadModel *downloadModel = self.downloadModels[SRFileName(URL)];
    if (downloadModel) {
...
- (void)cancelAllDownloads {
    ...
    for (DownloadModel *downloadModel in downloadModels) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (downloadModel.state) {
                downloadModel.state(DownloadStateCanceled);
            }
        });
    }
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler {
    ...
    DownloadModel *downloadModel = self.downloadModels[dataTask.taskDescription];
    if (!downloadModel) {
        return;
    }
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    ...
    DownloadModel *downloadModel = self.downloadModels[task.taskDescription];
    if (!downloadModel) {
        return;
    }
...
- (void)deleteAllFiles {
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSArray *fileNames = [fileManager contentsOfDirectoryAtPath:DownloadDirectory error:nil];
